...
app.post('/api/register', (req, res) => {
  ...
  if (String(password).length < 6) {
    return res.json({ success: false, message: 'ፓስዎርድ ቢያንስ 6 ፊደል ይሁን' });
  }

  if (!/^\d{9,15}$/.test(String(phone))) {
...
app.post('/api/deposit', authMiddleware, (req, res) => {
  ...
  if (typeof amount !== 'number') {
    return res.json({ success: false, message: 'amount number መሆን አለበት' });
  }
...
if (ADMIN_KEY === 'CHANGE_ME') {
  console.warn('⚠️ Set ADMIN_KEY in .env before production');
}
